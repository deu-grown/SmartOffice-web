...
| # | 발견 시점 | 결함 | 처리 위치 |
|---|-----------|------|-----------|
| 1 | 묶음 2 검증 (2026-05-14) | `GET /api/v1/dashboard/summary` HTTP 500 (DashboardService 내부 예외 추정) | `SmartOffice-server/BACKEND_SUGGESTIONS.md` #7. 백엔드 수정 플랜에서 처리. **마감 (백엔드 fix `d96e77d`, 2026-05-15)** — `DeviceRepository.countByDeviceStatus` enum 시그니처 변경 (Hibernate 7.2.7 type mismatch). /dashboard KPI 4종 (현재 출근 0/10·오늘 예약 5·활성 장치 18·전체 사용자 10) 복원 시각 검증 통과 |
| 2 | 묶음 3 검증 (2026-05-14) | `access_logs.authResult` 값 혼재 — V5 시드 잔존 `"ALLOW"` 8건 vs 코드 표준 `"APPROVED"` 53건 (`"DENIED"` / `"BLOCKED"` 도 함께 사용) | 묶음 4 검증에서 확인 완료. `SmartOffice-server/BACKEND_SUGGESTIONS.md` #8 등록. 백엔드 수정 플랜에서 V9 마이그레이션(`UPDATE access_logs SET auth_result='APPROVED' WHERE auth_result='ALLOW'`). 클라이언트는 `features/accesslog/types.ts` 의 literal union 에 `"ALLOW"` 호환 유지. **마감 (백엔드 fix `55a6c37` V9 마이그레이션, 2026-05-15)** — curl ALLOW 0건 / APPROVED 61건 정합. web 측 ALLOW literal 호환 제거 권장 SUGGESTIONS #2 append |
...
| 5 | 플랜 3-2 묶음 4 종료 (2026-05-14) | `GET /api/v1/power/zones/{zoneId}/hourly` HTTP 500 — POWER 미터 보유 zone (2/4/5/7) 모두 일관 실패. `startDate/endDate` 유무 무관. 동 컨트롤러의 `/billing` 은 정상 → `PowerService.getHourlyHistory` 만 결함 (HourlyPowerProjection mapping 또는 null 처리 추정) | `SmartOffice-server/BACKEND_SUGGESTIONS.md` #11 등록 (우선순위 **상**). web 측은 `usePowerHourly` isError + ErrorBoundary graceful handling 으로 동작 가능 (시각화만 비활성). 백엔드 수정 플랜에서 우선 처리. **마감 (백엔드 fix `bfe28be`, 2026-05-15)** — 진짜 원인은 `findHourlyPowerProjection` SELECT/GROUP BY DATE_FORMAT 패턴 불일치 (only_full_group_by). 마스터플랜 가설 1 (projection 매핑) 은 무관. GROUP BY 패턴을 SELECT 와 통일. /building 시간별 전력 + /zones 전력 탭 복원 시각 검증 통과 |
...
| 10 | 플랜 3-3 시각 재검증 (2026-05-14) | `parking_spots` 좌표 충돌 검증 백엔드 부재 — DB UNIQUE 제약 `(zone_id, position_x, position_y)` 부재 + `ParkingServiceImpl.createSpot`/`updateSpot` 좌표 충돌 검증 X. 같은 zone 같은 좌표에 spot 다수 등록 시 평면도 z-stack 으로 마지막만 노출(결함 11 root cause 재현). `position_x`/`position_y` null XOR 검증도 부재 — 좌표 시스템 의미상 "둘 다 null" 또는 "둘 다 값"만 정합 | 즉시 우회(web `fix(parking): 1d2fa0d`): `ParkingSpotsTable` 등록/수정 핸들러에 `validateCoordinates()` 사전 검증 추가. `useParkingSpots({})` 응답에서 같은 zone + 같은 좌표 spot 검색 → 충돌/`null XOR` 시 toast 차단. `SmartOffice-server/BACKEND_SUGGESTIONS.md` #16 등록 (우선순위 **중**, DB UNIQUE + Service 검증 + DTO null XOR 가드). 채택 시 `validateCoordinates` + `allSpotsQuery` 제거. race condition 보장은 서버 측에서만 가능. **마감 (백엔드 fix `7975c49` + `3101713`, 2026-05-15)** — V10 UNIQUE 제약 + Service `validatePosition` (null XOR + 좌표 충돌) + ErrorCode 신규 2건 + 통합 테스트 3 시나리오. curl 검증: 동일 좌표 409 "동일 구역 내 중복된 주차면 좌표입니다." + null XOR 400 "주차면 좌표는 X/Y 둘 다 입력하거나 둘 다 비워야 합니다.". web 측 후속: `ParkingSpotsTable.validateCoordinates()` 제거 권장 (BE UNIQUE + Service 검증 + 사용자 친화 메시지 완비, FE 중복 가드 불필요) |
...
| 마지막 (md + 머지 + push) | 완료 | 2026-05-15 | 2026-05-15 | 외주 단계 잔존 정정(title/name/favicon/라이선스/브랜드 라벨/@google/genai) + README 전면 갱신 + md 6종 동기화(web 3 + server 2 + 상위 3) + 마스터플랜 6절·9절 갱신. **web**: 7 커밋 + `--no-ff` merge + main 직접 push (`origin/main 7f087df`, 84 ahead). **server**: 7 커밋 + PR #26 머지(branch protection rule 발견으로 PR 분기, `origin/main 09cb092`, 15 ahead). 양쪽 feature 브랜치 삭제 완료. 합계 99 커밋(web 84 + server 15) origin 진입. 후속: 별도 plan mode 백엔드 수정 플랜 진입 — BACKEND_SUGGESTIONS #7~#16 + 잔존 결함 #1~#10 처리 |
