...
export const zoneHandlers = [
  ...
  http.put("/api/v1/zones/:id", async ({ request, params }) => {
    ...
    if (!target) {
      return HttpResponse.json({ code: "error", message: "구역을 찾을 수 없습니다.", data: null }, { status: 404 });
    }
  ...
  http.delete("/api/v1/zones/:id", ({ params }) => {
    ...
    if (hasChildren) {
      return HttpResponse.json(
        { code: "error", message: "하위 구역이 있는 구역은 삭제할 수 없습니다.", data: null },
        { status: 409 }
      );
    }
    zones.splice(idx, 1);
    return HttpResponse.json({ code: "success", message: "구역이 삭제되었습니다.", data: null });
  }),
];
