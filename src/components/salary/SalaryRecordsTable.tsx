// 급여 내역 표 (좌측 검색 + 행 + 우측 명세서 카드).
// 묶음 3 커밋 3.1 β 분할: 기존 SalaryManagement.tsx mock 통째 이식. 커밋 3.2 에서 features/salary 흡수 예정.
import { useState } from "react";
import { Search, Printer, FileDown, DollarSign } from "lucide-react";
...
export function SalaryRecordsTable() {
  ...
  const filteredEmployees = initialEmployees.filter(
    (emp) =>
      emp.name.toLowerCase().includes(searchTerm.toLowerCase()) || emp.employeeId.includes(searchTerm)
  );

  const formatCurrency = (amount: number) => new Intl.NumberFormat("ko-KR").format(amount);

  const handlePrint = () => toast.success("급여 명세서가 인쇄 대기열에 추가되었습니다.");
  const handleDownloadPdf = () => toast.success("명세서 PDF 다운로드가 시작되었습니다.");
            ...
            <TableBody>
              <AnimatePresence mode="popLayout">
                {filteredEmployees.map((record) => (
                  ...
                  >
                    <TableCell className="py-5 font-bold">{record.name}</TableCell>
